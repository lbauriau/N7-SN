...
for J= 1:N
    ...
    [X_VS, Y_VS, Alpha_VS, w, c,code_retour] = SVM_2_souple(X,Y,lambda);
    ...
    for i = 1:nb_donnees_test
        x_i = X_test(i,:);
        prediction = sign(exp(-sum((X_VS-x_i).^2,2))'*diag(Y_VS)*Alpha_VS-c);
...
figure;
plot(lambda_tab,pourcentage_tab);
ylabel ("Pourcentage de bonne classification");
xlabel("lambda");
title("Pourcentage de bonne classification en fonction de lambda");
