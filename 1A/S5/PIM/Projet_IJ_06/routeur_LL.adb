...
procedure routeur_LL is
	...
	--------------------------------
  	...
  	masque : Unbounded_String;
   	...
   	-- Sous programme
	type T_adresse_IP is array(1..4) of Integer;
	type T_table_elt is
		record
	...
	package lca_hachage is
			new lca_table (T_element =>T_table_elt);
	use lca_hachage;
	...
	procedure prc_afficher_adresse (ad : T_adresse_IP) is
	begin
		for i in 1..3 loop
			Put(ad(i));
			Put(".");
	...
	function fct_convertir_10_2_adresse ( adresse : T_adresse_IP) return T_adresse_bin is
	...
	function fct_convertir_2_10_adresse ( adresse_bin : T_adresse_bin) return T_adresse_IP is
   ...
   -----------------------------------------------------------------------------------------
	...
	function fct_comp (elt_table : T_table_elt; elt_paquet : T_adresse_IP) return boolean is
		adresse_bin : T_adresse_bin;
   ...
   ------------------------------------------------------------------------------------------------
	...
	function fct_masque_allonge (masque : in T_adresse_IP) return T_adresse_IP is
	...
	procedure prc_new_masque (elt : in out T_table_elt; table_hach : in T_LCA ) is
	begin
	...
	function fct_new_masque (elt : in T_table_elt; table_hach : in T_LCA) return T_adresse_IP is
	...
	procedure prc_ajouter_FIFO(cache : in out T_LCA; elt_cache : in T_table_elt) is
	begin
		if fct_taille(cache)= borne then
			prc_supprimer(cache);
		else
			null;
		end if;
		Enregistrer(cache, elt_cache,0);
	end prc_ajouter_FIFO;

	-- politique LRU
	
	procedure prc_ajouter_LRU(cache : in out T_LCA; elt_cache : in out T_table_elt; indice_cache : in out integer) is
	begin
		if fct_taille(cache) = borne then
			if not (indice_cache=0) then
				prc_deplacer_i_fin(cache,indice_cache);
			else
				null;
			end if;
			prc_supprimer(cache);
	...
	procedure prc_ajouter_LFU(cache : in out T_LCA; elt_cache : in out T_table_elt) is
		compteur : Integer := 1;
   	begin
		if not Est_vide(cache) then
			compteur := compteur + fct_freq(cache);
		end if;
      		if fct_taille(cache)= borne then
			prc_supprimer_LFU(cache);
      		else
        		null;
      		end if;
      		Enregistrer(cache, elt_cache, compteur);
   	end prc_ajouter_LFU;
   
	
	----------------------------------------------------------------------------------
		
	procedure prc_afficher_cache(cache : T_LCA) is
	begin
   		if not Est_Vide(cache) then
     			for I in 1..3 loop
        	 		Put(Integer'Image(Cache.Element.destination(I)));
				Put(".");
     		 	end loop;
			Put(Integer'Image(Cache.Element.destination(4)));
			Put(" ");
      			for I in 1..3 loop
         			Put(Integer'Image(Cache.Element.masque(I)));
				Put(".");
     			end loop;
			Put(Integer'Image(Cache.Element.masque(4)));
			Put(" ");
      			Put(Cache.Element.interf);
			New_Line;
     			prc_afficher_cache(cache.Suivant);
   		end if;
	end prc_afficher_cache;
	...
	exception
   	...
   	aux_table_hach : T_LCA;
   	resultat_lca : T_LCA;
   	destination_tb : T_adresse_IP;
   	masque_tb : T_adresse_IP;
   	interf_tb : Unbounded_String;
   	table_elt : T_table_elt;
   	l : integer;
   	taille_m : integer;
	new_masque : T_adresse_IP;
	elt_masque : T_table_elt;
	cache : T_LCA;
	aux_cache : T_LCA;
	...
	nb_demande :=0.0;
  	...
  	loop
		...
		-- Traitement d'une adresse
			...
			table_elt.masque := (0,0,0,0);
         		table_elt.interf :=To_Unbounded_String("0");
	 ...
	 --On choisit d'afficher la destination ainsi que l'interface
        ...
        -- Ici nous devons rajouter la recherche dans le cache 
				while (not Est_Vide(aux_cache)) loop
					k := k+1;
            				if fct_comp(aux_cache.element,destination_tb) and taille_m <= fct_taille_masque(fct_convertir_10_2_adresse(aux_cache.element.masque)) then
               					interf := aux_cache.element.interf;
               					taille_m := fct_taille_masque(fct_convertir_10_2_adresse(aux_cache.element.masque));
						new_masque := fct_new_masque(aux_cache.element,aux_table_hach);
						indice_cache := k;
					else
						null;
					end if;
            				aux_cache := aux_cache.suivant;      
         			end loop;
			end if;

			appartient_cache := (taille_m/=0);

			if not appartient_cache then
				aux_table_hach := table_hach;
				defaut_cache := defaut_cache+1.0;
         			while (not Est_Vide(aux_table_hach)) loop
            				if fct_comp(aux_table_hach.element,destination_tb) and taille_m <= fct_taille_masque(fct_convertir_10_2_adresse(aux_table_hach.element.masque)) then
               					interf := aux_table_hach.element.interf;
			...
			if not (taille_m = 0) then
				elt_masque.masque := new_masque;
				elt_masque.interf := interf;
				elt_masque.destination := fct_convertir_2_10_adresse (fct_masque_bin(fct_convertir_10_2_adresse (destination_tb), fct_convertir_10_2_adresse(new_masque)));

				if not appartient_cache or (taille_m /= fct_taille_masque(fct_convertir_10_2_adresse(new_masque))) then
					if politique = To_Unbounded_String("FIFO") then
						prc_ajouter_FIFO(cache,elt_masque);
					elsif politique = To_Unbounded_String("LRU") then
						prc_ajouter_LRU(cache, elt_masque,indice_cache);
					elsif politique = To_Unbounded_String("LFU") then
						prc_ajouter_LFU(cache, elt_masque);
					else
						null;
					end if;
				end if;
